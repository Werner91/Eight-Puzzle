...
public class EightPuzzle {
	...
	private void breathFirstSearch(ArrayList<int[]> nodeList, int[] goalNode, int depth, int maxDepth){
		
		if(depth > maxDepth){
			System.out.println("Reach max Depth: " + depth);
			return;
		}
		...
		//call BreathFirstSearch recursively
		if(!(childNodes.isEmpty())){
			breathFirstSearch(childNodes, goalNode, depth+1, maxDepth);
		}else{
	...
	private boolean depthFirstSearch(int[] node, int[] goalNode, int depth, int maxDepth){
		...
		if(depth > maxDepth){
			System.out.println("Reaches max Depth: " + depth);
			return false;
		}
		
		if(zielErreicht(node)){
			System.out.println("Solution found! Deoth: !" + depth);
			return true;
		}
	...
	public static void main(String[] args){
		...
		puzzle.breathFirstSearch(nodeList, goalNode, 0, maxDepth);
